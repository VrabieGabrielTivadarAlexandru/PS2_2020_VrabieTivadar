...
void ocolire(){				//Parte din modulul de ocolire
	curba_var(80,100);		//iesi de pe banda curenta, spre stanga
	_delay_ms(500);
	curba_var(100,80);		//intra pe cealalta banda, rotire spre dreapta
	_delay_ms(500);
	fata();					//mergi in fata 1 secunda
	_delay_ms(1000);
	curba_var(100,80);		//incheie depasirea, initiaza intrarea pe banda normala, rotire spre dreapta
	_delay_ms(500);
	curba_var(80,100);		//intra pe banda normala, rotire spre stanga
	_delay_ms(500);
}
...
ISR(INT1_vect){			//Parte din modulul de ocolire
	timp_initial = TCNT1;
	while (PIND & 0X08) {
